...
napi_value InputTest(napi_env env, napi_callback_info info)
{
  napi_status status;
...
napi_value Init(napi_env env, napi_value exports)
{
  ...
  // adding input test to export
  status = napi_create_function(env, NULL, 0, InputTest, NULL, &fn);
  if (status != napi_ok)
  {
    napi_throw_error(env, NULL, "Unable to wrap native function");
  }

  status = napi_set_named_property(env, exports, "InputTest", fn);
  if (status != napi_ok)
  {
    napi_throw_error(env, NULL, "Unable to populate exports");
  }
